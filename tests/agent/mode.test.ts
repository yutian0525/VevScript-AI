...
describe('filterSchemasForMode / getToolSchemas', () => {
  it('agent 模式返回全量 37 个（schemas.ts 当前 37 工具）', () => {
    expect(getToolSchemas('agent')).toHaveLength(37);
    expect(getToolSchemas()).toHaveLength(37); // 缺省 = agent
  });
...
describe('记忆 cap 过滤与守卫', () => {
  ...
  it("cap='off' → 三个都不下发，其余工具不受影响", () => {
    ...
    expect(names).toHaveLength(34); // 37 全量 − 3 个记忆工具
  });
