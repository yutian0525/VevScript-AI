...
//   "description": 键与语法开销（37 工具 + 135 参数） ≈2,394  ← 动不了
...
// 17,700 = 17,384（Task 7 后实测）+ 316 余量。Task 7 实际花了 207（query 参数 + 描述增句 123、
// 评审回补 84），旧闸 17,400 只剩 16 字符——任何措辞微调都撞闸，饱和的绊线拦不住真回归，
// 只锁死合法修改。闸门仍比优化前的 19,745 低约 10%，滑回旧态照样触发。
// **不要**为了「把数字压回 15,800」而删操作要点或参数语义。
const BUDGET_TOTAL = 17_700;
