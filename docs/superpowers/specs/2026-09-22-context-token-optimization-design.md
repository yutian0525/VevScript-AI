...
| 5 | schema 瘦身力度 | **中等**：删重复与设计论证、合并工具级/参数级重复描述、压长枚举与示例；保留全部操作要点与用户可感知后果 | 用户先选保守，在 §0.1 测出 57% 是结构管道、保守实际只能降 -12% 后**改选中等**。预估 -23%，**实测 -13%**（§5.4 勘误：预估的公式与保留清单矛盾）。中等档仍不删任何操作要点，只删重复与论证，风险可控 |
...
**目标**（数字已按 §0.1 的实测构成与 §5.4 的勘误修正；工具字符数均为序列化后口径）
...
> ⚠️ 上面这个 `× 0.45` 的推导**在实施中被证伪**，见 §5.4。它是按「全部散文统一砍 55%」算的，而 §5.1 的保留清单要求参数级 description 只删与工具级重复的部分——两条规则互相矛盾。以保留清单为准。
...
- 全部 schema 序列化字符数 ≤ **17,700**（原 19,745；实施实测落到 17,384，见 §5.4）
- 单工具 ≤ **1,400** 字符（原最大 1,745；实施实测最大 1,387）

超限即测试失败，把「描述又写长了」变成 CI 可见的回归。**这个阈值是回归绊线，不是目标**——它设在实测落点之上约 1%，为 Task 7 给 `list_skills` 加 `query` 参数（约 +60 字符）留出余量。

### 5.4 勘误：中等档的实际落点与原因（2026-09-22 实施后追记）

**实测：19,745 → 17,177 字符（-13%），不是预估的 -23%。**

原因：§5.1 与 §5.2 互相矛盾。§5.1 的保留清单明确要求「留 参数级 description——它是模型填参的唯一依据，只删与工具级重复的部分」；而 §5.2 的 `地板 + 散文 × 0.45` 是按「全部散文统一砍 55%」推导的，其中包含参数级。两条规则不能同时成立。

**以保留清单为准**，因为它是用户明确的优先级（宁少省字，不降工具选择质量）的编码。实际落点：

| 层 | 前 | 后 | 降幅 |
|---|---|---|---|
| 工具级 description | 6,060 | 3,815 | -37% |
| 参数级 description | 2,391 | 2,080 | -13% |
| 结构管道 + `"description":` 键开销 | 11,294 | 11,283 | 0（不可削） |
| **合计** | **19,745** | **17,177** | **-13%** |

参数级只降 13%，是因为剩下的每个字符都是填参语义（`level` 的枚举含义、`缺省 5，上限 20`、`来自 list_pages`、`越界报错` 之类），删它等于让模型猜参数。

**要再往下压，只能删参数语义或减少工具数**——而 §0.1 已经指出后者才是真正的杠杆。本设计不做（懒加载已被否决）。

**对 §2 目标的影响**：agent 模式的固定开销降幅由 -20% 修正为约 -10%；ask 模式的 -46% 不受影响（它靠砍工具数，不靠改描述）。**这条修正过的事实必须在交付时告知用户**——它改变了最初承诺的收益量级。

**后续追记（Task 7 之后）**：上表是 Task 6 的落点（17,177）。Task 7 给 `list_skills` 加 `query` 参数并回补了三条被误删的工具选择线索，总量升至 **17,384**，故 §5.3 的闸门随之定为 17,700（余量 316）。
...
- 参数加 `query?: string`。
- 命中 0 个时**不返回空列表**——返回诊断：`{ skills: [], hint: '无匹配「xxx」；检索范围内共 N 个，前 10 个是：<command 列表>' }`（沿用 `query_page` 命中 0 个给改法的思路，避免模型读到空列表以为技能库是空的）。
  - **N 是「检索范围内」而非「技能库」**（实施期修订）：`enabled` 过滤发生在检索之前，故 N 是被检索子集的基数。原稿写「技能库共 N 个」是过度声明——而这条 hint 直接喂给写技能前的查重判断，且写入链路没有第二道重复检测，说错了模型无从核对。前 10 个列表同样取自过滤后集合，所以改口径而非改计数。
- 命中时返回全部命中（已按分排序），不截断。
...
- `tests/agent/schema-budget.test.ts`：总量 ≤ 17,700、单工具 ≤ 1,400（**必须基于序列化后的 schema，不能基于源码文本**——见 §0.1 测量方法备注）。
