...
1. agent 模式工具 schema：19,745 → ~17,180 字符（**-13%**；实施后实测落点，原估 -23% 已作废，见 §5.4）。
2. agent 模式新对话固定开销：21,708 → ~19,400 字符（**-11%**；同上）。
...
- 全部 schema 序列化字符数 ≤ **17,400**（原 19,745；实施实测落到 17,177，见 §5.4）
...
**对 §2 目标的影响**：agent 模式的固定开销降幅由 -20% 修正为约 -11%；ask 模式的 -46% 不受影响（它靠砍工具数，不靠改描述）。**这条修正过的事实必须在交付时告知用户**——它改变了最初承诺的收益量级。
