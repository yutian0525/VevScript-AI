...
const GM_INSTALLS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['GM_openInTab', 'function (url, opts) { opts = opts || {}; var h = { closed: false, onclose: null, close: function () { __GM_post("CloseTab", [h.__tabId]); } }; __GM_post("OpenInTab", [url, opts]).then(function (tabId) { h.__tabId = tabId; __GM_listeners.set("tab:" + tabId, function (d) { h.closed = !!d.closed; if (d.closed && h.onclose) h.onclose(); }); }); return h; }'],
