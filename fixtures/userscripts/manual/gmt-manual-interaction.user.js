...
(function () {
  var actions = {
    ...
    'i3-clip': function (log) {
      GM.setClipboard('gmt-manual-clipboard-' + (Date.now() % 10000)).then(function () {
        ...
        GMT.log('i3', '写入失败（MV3 SW 无手势链可能被拒）：' + ((e && e.message) || e));
      });
    }
  };
  ...
  GMT.render({
    ...
    cards: [
      ...
      { id: 'i3', api: 'GM_setClipboard(data)', desc: '写剪贴板仅文本；SW 无手势链时可能被拒（失败给可读提示）。', steps: [{ id: 'i3-clip', label: '写剪贴板' }, '把内容粘贴到任意文本框核对前缀'], expect: '粘贴出 gmt-manual-clipboard-* 文本（或失败行给可读错误）' },
